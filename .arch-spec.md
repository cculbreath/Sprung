...
**Access Patterns**:
- Components access stores via `OnboardingDependencyContainer` for read operations
- StateCoordinator handles coordination/orchestration (event subscriptions, phase transitions)
- Write operations that affect multiple stores should go through events
- Direct store access for simple queries is preferred over delegation methods
