...
## Core Tenets
1. **StateCoordinator (actor) is the single point of coordination** - All state access flows through it, delegating to domain-specific stores
...
- **COORDINATES**: All state access flows through StateCoordinator as the single entry point
- **DELEGATES TO**: Domain-specific stores (ObjectiveStore, ArtifactRepository, ChatTranscriptStore, StreamQueueManager, LLMStateManager, SessionUIState, DraftKnowledgeStore)
- **OWNS DIRECTLY**: currentPhase, wizardStep, evidenceRequirements (cross-cutting state that doesn't fit a single domain)
...
### Domain-Specific Stores (actors)
StateCoordinator delegates to specialized stores for domain isolation and testability:
...
These stores are injected into StateCoordinator and accessed only through it (not directly by handlers).
...
// Bypassing StateCoordinator to access domain stores directly
class MyHandler {
    let objectiveStore: ObjectiveStore
    func check() async {
        await objectiveStore.getStatus("id") // ❌ Must go through StateCoordinator
    }
}
...
// StateCoordinator delegates to domain stores (internal implementation)
actor StateCoordinator {
    private let objectiveStore: ObjectiveStore
    private let llmStateManager: LLMStateManager

    // ✅ ALLOWED: StateCoordinator delegates to specialized stores
