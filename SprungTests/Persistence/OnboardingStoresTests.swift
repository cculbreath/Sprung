...
// MARK: - InferenceGuidanceStore

@MainActor
final class InferenceGuidanceStoreTests: InMemoryStoreCase {

    private func makeGuidance(
        nodeKey: String = "objective",
        source: GuidanceSource = .auto,
        enabled: Bool = true
    ) -> InferenceGuidance {
        InferenceGuidance(
            nodeKey: nodeKey,
            displayName: "Objective Guidance",
            prompt: "Write a strong objective.",
            source: source,
            isEnabled: enabled
        )
    }

    func testAddAndQueryByNodeKey() throws {
        let store = InferenceGuidanceStore(context: context)
        store.add(makeGuidance(nodeKey: "custom.jobTitles"))

        XCTAssertEqual(store.allGuidance.count, 1)
        XCTAssertNotNil(store.guidance(for: "custom.jobTitles"))
        XCTAssertNil(store.guidance(for: "nonexistent"))
    }

    func testDisabledGuidanceExcludedFromQueries() throws {
        let store = InferenceGuidanceStore(context: context)
        store.add(makeGuidance(nodeKey: "objective", enabled: false))
        XCTAssertEqual(store.allGuidance.count, 1)
        XCTAssertNil(store.guidance(for: "objective")) // guidance(for:) filters to enabled
        XCTAssertEqual(store.enabledGuidance.count, 0)
    }

    func testToggleEnabled() throws {
        let store = InferenceGuidanceStore(context: context)
        let guidance = makeGuidance(enabled: true)
        store.add(guidance)
        store.toggleEnabled(guidance)
        XCTAssertFalse(guidance.isEnabled)
        XCTAssertEqual(store.enabledGuidance.count, 0)
    }

    func testGuidanceMatchingWildcardPattern() throws {
        let store = InferenceGuidanceStore(context: context)
        store.add(makeGuidance(nodeKey: "experience.job1.bullets"))
        let matched = store.guidanceMatching(pattern: "experience.*.bullets")
        XCTAssertNotNil(matched)
        XCTAssertEqual(matched?.nodeKey, "experience.job1.bullets")
    }

    func testDeleteAutoGeneratedLeavesUserGuidance() throws {
        let store = InferenceGuidanceStore(context: context)
        store.add(makeGuidance(nodeKey: "auto-node", source: .auto))
        store.add(makeGuidance(nodeKey: "user-node", source: .user))
        XCTAssertTrue(store.hasAutoGeneratedGuidance)

        store.deleteAutoGenerated()
        XCTAssertEqual(store.allGuidance.count, 1)
        XCTAssertEqual(store.allGuidance.first?.nodeKey, "user-node")
        XCTAssertFalse(store.hasAutoGeneratedGuidance)
    }

    func testRenderedPromptSubstitutesAttachments() throws {
        let store = InferenceGuidanceStore(context: context)
        ...
        store.add(guidance)
        XCTAssertEqual(store.renderedPrompt(for: "objective"), "Use these: [\"a\"]")
    }
}
