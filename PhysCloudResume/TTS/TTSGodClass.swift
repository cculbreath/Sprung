...
struct CoverLetterAiContentView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            print("AI content")
            // Set up buffering state callback
            setupTTSCallbacks()
        }
    ...
    // MARK: - TTS Callbacks Setup

    /// Set up callbacks for the TTS provider to handle buffering state changes
    private func setupTTSCallbacks() {
        // Connect to the buffering state callback
        ttsProvider.onBufferingStateChanged = { [self] buffering in
            DispatchQueue.main.async {
                self.isBuffering = buffering
                print("Buffering state changed: \(buffering)")
            }
        }
    }
    ...
    private func startStreamingFromBeginning() {
        ...
        // Note: We no longer need to manually set isBuffering here as it will be set by the TTSProvider
        // through the onBufferingStateChanged callback
        ...
        ttsProvider.streamAndPlayText(
            ...
            onStart: {
                ...
                self.isPaused = false
                // Note: isBuffering will be updated via the onBufferingStateChanged callback
            },
            onComplete: { error in
                DispatchQueue.main.async {
