...
//
//  The two callers differ in exactly three parameters, captured by this value
//  type:
...
//  - includeDocumentBreakpoint: onboarding plants a breakpoint on the last
//    document block (caching through the largest payload covers all earlier
//    blocks); revision keeps its single PDF in the front-loaded prefix and does
//    not need one.
...
struct AnthropicCacheBreakpointPlanner {
    ...
    /// Whether to plant a breakpoint on the last document block in the array.
    let includeDocumentBreakpoint: Bool
    ...
    /// 2. Document (conditional) — last document block in the array.
    /// 3. Lookback (conditional, totalBlocks > 20) — last markable block of the
    ...
    func plan(messages: [AnthropicMessage]) -> [AnthropicMessage] {
        ...
        // Document (conditional) — the LAST document block in the array: caching
        // through it covers all earlier blocks, including the document payloads
        // themselves. Collapses into the tail when they coincide.
        var document: BlockPosition?
        if includeDocumentBreakpoint {
    ...
    /// are supplied in priority order (tail, document, lookback), so on overflow the
    /// lookback breakpoint is dropped first, then the document breakpoint; the tail
    /// breakpoint always survives. Duplicate positions collapse. Pure function so
    /// the invariant is inspectable in isolation.
