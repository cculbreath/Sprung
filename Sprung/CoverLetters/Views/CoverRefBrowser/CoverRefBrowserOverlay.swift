...
/// Floating overlay panel for browsing, editing, and managing cover letter references.
/// Features a coverflow-style card deck with filter chips and CRUD controls.
struct CoverRefBrowserOverlay: View {
    ...
    private var headerSection: some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("\(cards.count) reference\(cards.count == 1 ? "" : "s") total")
                    .font(.caption)
    ...
    private func countForFilter(_ filter: CardTypeFilter) -> Int {
        ...
        case .all:
            return cards.count
        case .dossier:
            return cards.filter { $0.isDossier }.count
        case .backgroundFacts:
            return cards.filter { $0.type == .backgroundFact && !$0.isDossier }.count
        case .writingSamples:
            return cards.filter { $0.type == .writingSample }.count
        }
    }
    ...
    private let visibleCardOffset: CGFloat = 220
    ...
    @ViewBuilder
    private func coverflowCard(card: CoverRef, index: Int, offset: Int) -> some View {
    ...
    private func deleteCard(_ card: CoverRef) {
    ...
    private func exportCardsAsJSON() {
        ...
        panel.nameFieldStringValue = "cover-refs.json"
        panel.title = "Export Cover Letter References"
        panel.message = "Choose a location to save the references JSON file."
        ...
        panel.begin { response in
            ...
            Task { @MainActor in
                do {
                    ...
                    Logger.info("Exported \(cards.count) cover refs to \(url.path)", category: .general)
                } catch {
                    Logger.error("Failed to export cover refs: \(error)", category: .general)
                }
            }
        }
    }
