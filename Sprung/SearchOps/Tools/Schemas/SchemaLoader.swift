...
enum SchemaLoader {
    ...
    /// - Parameter resourceName: The name of the JSON file (without extension) in Resources/SearchOpsSchemas/
    /// - Returns: A JSONSchema object constructed from the JSON file
    static func loadSchema(resourceName: String) -> JSONSchema {
        guard let url = Bundle.main.url(forResource: resourceName, withExtension: "json", subdirectory: "SearchOpsSchemas") else {
            fatalError("Schema resource '\(resourceName).json' not found in SearchOpsSchemas directory")
        }
