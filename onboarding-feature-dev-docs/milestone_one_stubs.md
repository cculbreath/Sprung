...
- `Sprung/Onboarding/Core/OnboardingInterviewService.swift:23` – `pendingExtraction` is tracked but never populated. Future milestones need presenter methods (mirroring upload/validation) so the UI cards can surface the extraction review flow.
...
- `Sprung/Onboarding/Views/Components/ResumeSectionsToggleCard.swift` assumes the service will provide toggle requests, but no runtime path currently feeds them.
